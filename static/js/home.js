...
async function fetchHomeData() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            {
                console.warn("서버에 오류가 있습니다. 다음에 다시 시도 바랍니다.");            
            }
        }
    } catch (error) {
...
        if (modal) {
            modal.classList.add('visible'); // opacity와 display를 CSS로 제어
        }
    }, 10);
}
...
        
        // (선택) 애니메이션(0.2s)이 끝난 후 DOM에서 완전히 제거
        setTimeout(() => {
            modal.remove();
        }, 200); 
    }
